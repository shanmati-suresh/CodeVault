<?php
    if(isset($_COOKIE['myservername']) && isset($_COOKIE['myusername']) && isset($_COOKIE['username']) && isset($_COOKIE['password']) ){
        $myservername = $_COOKIE['myservername'];
        $myusername = $_COOKIE['myusername'];
        $mypassword = $_COOKIE['mypassword'];
        $mydb = $_COOKIE['mydb'];
        $username = $_COOKIE['username'];
        $user_id = $_COOKIE['user_id'];
        
        $conn = new mysqli($myservername, $myusername, $mypassword, $mydb);

        // Check connection
        if ($conn->connect_error) {
            die("Connection failed: " . $conn->connect_error);
        }

        $sql = "SELECT repo_name,username,c.commit_id FROM repositories r,users u,commits c,reviews re where c.user_id=u.user_id and c.repo_id=r.repo_id and c.commit_id=re.commit_id and re.status='pending' and r.user_id=$user_id ";
        $result = $conn->query($sql);

        // Check if the query returned any rows
        $data=[];
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                $repo_name = $row["repo_name"];
                $username= $row["username"];
                $commit_id= $row["commit_id"];
                $row_data = array('repo_name' => $repo_name,'username' => $username,'commit_id' => $commit_id);
                $data[]=$row_data;
            }
            echo json_encode($data);
        } else {
            echo json_encode($data);
        }
    } else {
        // Cookies not set or expired
        echo "Cookies not set or expired.";
    }


    
?>